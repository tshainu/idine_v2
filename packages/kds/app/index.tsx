...
function elapsed(date: string) { const sec = Math.max(0, Math.floor((Date.now() - new Date(date).getTime()) / 1000)); return sec >= 60 ? `${Math.floor(sec / 60)}m ${sec % 60}s` : `${sec}s`; }
function urgency(date: string) { const min = (Date.now() - new Date(date).getTime()) / 60000; return min >= 15 ? C.danger : min >= 8 ? C.warning : C.success; }
...
export default function KdsScreen() {
  ...
  return <View style={s.root}>
    ...
    {loading && !orders.length ? <View style={s.center}><ActivityIndicator size="large" color={C.gold} /></View> : display.length === 0 ? <View style={s.center}><Ionicons name="restaurant-outline" size={62} color={C.dim} /><Text style={s.empty}>All clear! No pending orders.</Text></View> : <FlatList data={display} keyExtractor={o => String(o.id)} numColumns={4} contentContainerStyle={s.grid} columnWrapperStyle={s.row} renderItem={({ item }) => <Ticket order={item} busy={busy === item.id} onReady={() => markReady(item.id)} />} />}
  </View>;
}

function Ticket({ order, busy, onReady }: { order: Order; busy: boolean; onReady: () => void }) {
  const color = TYPE[order.type || ""] || C.border;
  return <View style={[s.ticket, { borderColor: color }]}><View style={[s.ticketHead, { backgroundColor: `${color}22` }]}><View><Text style={s.orderNo}>{order.orderNumber}</Text><Text style={s.ticketSub}>{order.type === "dine-in" ? `Table ${order.tableId || "—"}` : (order.type || "ORDER").toUpperCase()}</Text></View><View style={s.timeBox}><Text style={[s.elapsed, { color: urgency(order.createdAt) }]}><Ionicons name="time-outline" size={13} color={urgency(order.createdAt)} /> {elapsed(order.createdAt)}</Text><Text style={s.ticketTime}>{new Date(order.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</Text></View></View><View style={s.items}><Text style={s.itemsLabel}>ITEMS</Text>{(order.items || []).map(i => <View key={i.id} style={s.item}><Text style={s.qty}>{i.qty}×</Text><View style={{ flex: 1 }}><Text style={s.itemName} numberOfLines={2}>{i.name}</Text>{i.note ? <Text style={s.note}>{i.note}</Text> : null}</View></View>)}{order.customerName ? <Text style={s.meta}>Customer: {order.customerName}</Text> : null}{order.placedBy ? <Text style={s.meta}>Waiter: {order.placedBy}</Text> : null}{order.notes ? <Text style={s.warning}>⚠ {order.notes}</Text> : null}</View><Pressable disabled={busy} onPress={onReady} style={[s.ready, busy && { opacity: .6 }]}>{busy ? <ActivityIndicator color="#fff" /> : <><Ionicons name="checkmark-circle-outline" size={17} color="#fff" /><Text style={s.readyText}>Cooked · Alert Waiter</Text></>}</Pressable></View>;
}

const s = StyleSheet.create({ root: { flex: 1, backgroundColor: C.bg }, center: { flex: 1, backgroundColor: C.bg, alignItems: "center", justifyContent: "center", gap: 12 }, muted: { color: C.muted, fontSize: 15 }, brandBox: { flexDirection: "row", alignItems: "center", gap: 10 }, logo: { width: 40, height: 40, borderRadius: 12, backgroundColor: C.gold, alignItems: "center", justifyContent: "center" }, brand: { color: C.gold, fontWeight: "800", fontSize: 17 }, subtitle: { color: C.dim, fontSize: 11, marginTop: 2 }, header: { height: 70, paddingHorizontal: 20, backgroundColor: C.surface, borderBottomWidth: 1, borderBottomColor: C.border, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 16 }, stations: { flex: 1, flexDirection: "row", justifyContent: "center", gap: 7 }, station: { paddingHorizontal: 13, paddingVertical: 9, borderRadius: 8, backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border }, stationActive: { backgroundColor: C.gold, borderColor: C.gold }, stationText: { color: C.muted, fontSize: 11, fontWeight: "700" }, stationActiveText: { color: C.bg }, headerRight: { flexDirection: "row", alignItems: "center", gap: 13 }, clock: { color: C.muted, fontSize: 13, fontWeight: "700" }, active: { backgroundColor: `${C.success}33`, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 7 }, activeText: { color: C.success, fontSize: 11, fontWeight: "800" }, grid: { padding: 16, paddingBottom: 24 }, row: { gap: 14, alignItems: "flex-start" }, ticket: { flex: 1, minWidth: 235, maxWidth: 360, backgroundColor: C.surface, borderWidth: 2, borderRadius: 15, overflow: "hidden", marginBottom: 14 }, ticketHead: { paddingHorizontal: 13, paddingVertical: 11, flexDirection: "row", justifyContent: "space-between" }, orderNo: { color: C.gold, fontSize: 17, fontWeight: "900" }, ticketSub: { color: C.muted, fontSize: 10, marginTop: 2 }, timeBox: { alignItems: "flex-end" }, elapsed: { fontSize: 11, fontWeight: "800" }, ticketTime: { color: C.dim, fontSize: 10, marginTop: 4 }, items: { padding: 13, gap: 7 }, itemsLabel: { color: C.dim, fontSize: 10, fontWeight: "800", marginBottom: 3 }, item: { flexDirection: "row", gap: 8 }, qty: { color: C.gold, fontSize: 14, fontWeight: "900", width: 27 }, itemName: { color: C.text, fontSize: 13, fontWeight: "600" }, note: { color: C.warning, fontSize: 10, marginTop: 2 }, meta: { color: C.muted, fontSize: 10, borderTopWidth: 1, borderTopColor: C.border, paddingTop: 6 }, warning: { color: C.warning, backgroundColor: `${C.warning}22`, borderRadius: 5, padding: 6, fontSize: 10 }, ready: { margin: 13, marginTop: 0, borderRadius: 10, paddingVertical: 11, backgroundColor: C.success, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7 }, readyText: { color: "#fff", fontSize: 12, fontWeight: "900" }, empty: { color: C.dim, fontSize: 17, fontWeight: "700" }, error: { backgroundColor: `${C.danger}22`, padding: 8, alignItems: "center" }, errorText: { color: C.danger, fontSize: 12 }, title: { color: C.text, fontSize: 22, fontWeight: "800" }, setupRow: { flexDirection: "row", gap: 14, flexWrap: "wrap", justifyContent: "center" }, selectCard: { width: 210, minHeight: 110, backgroundColor: C.surface, borderWidth: 1, borderColor: C.border, borderRadius: 16, alignItems: "center", justifyContent: "center", gap: 10 }, selectText: { color: C.text, fontWeight: "700", fontSize: 15 } });

...
export default function SettlementReport() {
  ...
  const periodOrders = orders.filter(order => {
    const timestamp = toMs(order.createdAt);
    const start = from ? new Date(`${from}T00:00:00`).getTime() : 0;
    const end = to ? new Date(`${to}T23:59:59.999`).getTime() : Number.MAX_SAFE_INTEGER;
    return timestamp >= start && timestamp <= end && ["completed", "billed", "paid"].includes(String(order.status || "").toLowerCase());
  });
  const invoicedSales = periodOrders.reduce((sum, order) => sum + Number(order.total || 0), 0);
  const unsettledSales = invoicedSales - totals.billed;
  ...
  const openPrint = (savePdf = false) => {
    ...
    win.document.write(`<!doctype html><html><head><title>Settlement Report</title><style>body{font-family:Arial,sans-serif;color:#111;margin:28px}h1{font-size:22px;margin:0 0 4px}p{color:#555;margin:4px 0 16px}.meta{display:flex;gap:28px;margin:14px 0;font-size:12px}.cards{display:flex;gap:12px;margin:14px 0;flex-wrap:wrap}.card{border:1px solid #ccc;padding:10px 14px;min-width:150px}.label{font-size:11px;color:#666}.value{font-size:16px;font-weight:bold;margin-top:5px}table{border-collapse:collapse;width:100%;font-size:11px}th,td{border-bottom:1px solid #ddd;padding:8px;text-align:left}th{background:#f2f2f2}.num{text-align:right}.ok{color:#14833b}.bad{color:#b42318}.total{font-weight:bold;border-top:2px solid #111}@media print{body{margin:12mm}}</style></head><body><h1>Settlement Report</h1><p>iDine register settlements</p><div class="meta"><span><b>Period:</b> ${esc(from || "All")} to ${esc(to || "All")}</span><span><b>Login:</b> ${esc(user?.name || user?.username || "Staff")}</span><span><b>Generated:</b> ${esc(new Date().toLocaleString())}</span></div><div class="cards"><div class="card"><div class="label">Settlement records</div><div class="value">${filtered.length}</div></div><div class="card"><div class="label">Total invoiced sales</div><div class="value">${money(invoicedSales)}</div></div><div class="card"><div class="label">Settlement billed</div><div class="value">${money(totals.billed)}</div></div><div class="card"><div class="label">Total settled</div><div class="value">${money(totals.settled)}</div></div><div class="card"><div class="label">Unsettled sales</div><div class="value">${money(unsettledSales)}</div></div></div><table><thead><tr><th>Date & time</th><th>Login user</th><th class="num">Billed</th><th class="num">Settled</th><th class="num">Difference</th><th>Justification</th></tr></thead><tbody>${rows || `<tr><td colspan="6">No settlement records for this period.</td></tr>`}<tr class="total"><td colspan="2">SETTLEMENT TOTAL</td><td class="num">${money(totals.billed)}</td><td class="num">${money(totals.settled)}</td><td class="num">${money(difference)}</td><td></td></tr></tbody></table><p><b>Invoiced sales:</b> ${money(invoicedSales)} &nbsp; <b>Unsettled sales:</b> ${money(unsettledSales)}</p><script>window.onload=()=>window.print()</script></body></html>`);
    win.document.close();
    void savePdf;
  };
  ...
  return <div className="flex h-screen overflow-hidden" style={{ background: BG }}>
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">{[{ label: "Settlement records", value: filtered.length }, { label: "Total invoiced sales", value: money(invoicedSales) }, { label: "Settlement billed", value: money(totals.billed) }, { label: "Total settled", value: money(totals.settled) }, { label: "Unsettled sales", value: money(unsettledSales) }, { label: "Settlement difference", value: money(difference) }].map(card => <div key={card.label} className="rounded-xl border p-4" style={{ background: SURF, borderColor: BORD }}><div className="text-xl font-bold" style={{ color: card.label === "Unsettled sales" && unsettledSales !== 0 ? "var(--color-danger)" : card.label === "Settlement difference" && difference !== 0 ? "var(--color-danger)" : GOLD }}>{card.value}</div><div className="text-xs mt-1" style={{ color: MUTED }}>{card.label}</div></div>)}</div>
